...
@Stateless
public class LOSFuelOrderLogServiceBean extends
		BasicServiceBean<LOSFuelOrderLog> implements LOSFuelOrderLogService {

	Logger log = Logger.getLogger(LOSFuelOrderLogServiceBean.class);

	public LOSFuelOrderLog create(VehicleData vehicle,
			LOSStorageLocation storLoc, int stationPump,
			LOSOrderReceipients receipient, OrderReceiptPosition rcptPos,
		...
		if (receipient != null) {
			...
			String receipientName = receipient.getRankAbbr() + " "
					+ receipient.getFirstName() + " "
					+ receipient.getLastName();
	...
	public LOSFuelOrderLog create(LOSStorageLocation storLoc,
			LOSOrderReceipients receipient, ItemData it, BigDecimal amount,
			String orderType, BigDecimal tankRemaining) {
